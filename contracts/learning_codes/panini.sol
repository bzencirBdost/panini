 ...
 */
 contract PaniniCardCreater is AttachingPaniniController {
  ...
  function generateRandomBaseId() public view returns (uint256) {
...
contract UsingCard is PaniniERC721Token, PaniniBase {
  ...
  function _addRandomCardToAddress(address _address) internal returns(uint256) {
    uint256 baseId = paniniCardCreater.generateRandomBaseId();
    //kart'in uretilmesi.
...
contract UsingCardPackage is UsingCard {
  ...
  event PackageCreated(uint256 id, address receiver, uint256 cardId1, uint256 cardId2, uint256 cardId3, uint256 cardId4, uint256 cardId5 );
...
/*
TODO: gerek yok. emit ile bu is server'da cozulur
//1 emit yeterli (owner, card1, card2, card3, card4, card5)
//Direk paket almak olacak.
  //created packages;
...
*/
  ...
  function createPackage(address _address) public {
    ...
    //generating cards
    uint256 cardId1 = _addRandomCardToAddress(_address);
    uint256 cardId2 = _addRandomCardToAddress(_address);
    uint256 cardId3 = _addRandomCardToAddress(_address);
    uint256 cardId4 = _addRandomCardToAddress(_address);
    uint256 cardId5 = _addRandomCardToAddress(_address);
    PackageCreated(packageId, _address, cardId1, cardId2, cardId3, cardId4, cardId5);
  }
...
//    players[msg.sender].owner = msg.sender;
    ...
    //5x card 
    _addRandomCardToAddress(msg.sender);
    _addRandomCardToAddress(msg.sender);
    _addRandomCardToAddress(msg.sender);
    _addRandomCardToAddress(msg.sender);
    _addRandomCardToAddress(msg.sender);
...
//sebebi: paninistate -> attachPaniniController
